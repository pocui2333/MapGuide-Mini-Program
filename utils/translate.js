export const replaceChineseNums = (str) => {
  const chineseNumRegex = /[一二三四五六七八九十]+/g;
  const matches = str.match(chineseNumRegex);
  ...
  matches.forEach((match) => {
    const num = chineseToNumber(match);
    replaced = replaced.replace(match, num);
  });

  return replaced;
}

const chineseToNumber = (digit) => {
